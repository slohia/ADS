...
class ADSClient:
    ...
    def extract_system_performance_data(self):
        try:
            ...
            system_dict['disk']['write_byte_rate'] = self.sys_mon.write_bytes()
            self.log.log_msg("Fetching read_byte_rate")
            system_dict['disk']['read_byte_rate'] = self.sys_mon.read_bytes()
    ...
    def transfer_to_server(self, path_to_xml):
        try:
            ...
            self.ssh.transfer_file_client_to_ads_server(ssh_client, remote_path, path_to_xml)
        except Exception, e:
